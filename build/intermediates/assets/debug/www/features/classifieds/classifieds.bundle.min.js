...
$scope.my_shop_view=val;return contentUpdated()};$scope.$on("$stateChangeSuccess",function(event,toState,toParams,fromState,fromParams){if(toState.name=="classifieds"&&!(/^classifieds/.test(fromState.name))){$scope.my_shop_view=!1;$timeout(function(){$scope.reloadContent(!1)},250)}});function filter_ads(){if($scope.my_shop_view){$scope.filtered_ads=_.orderBy(_.filter($scope.all_ads,{mine:!0}),["created_at_utc"],["desc"])}else{var ads=_.filter($scope.all_ads,{status:"published"});var so=_(cleaned_search_options());var order=so.get("order");var order_by=so.get("order_by");var order_dir=so.get("order_dir");$scope.filtered_ads=_.orderBy(_.filter(ads,function(ad){var show_ad=!0;function escapeRegExp(str){return str.replace(/[\-\[\]\/\{\}\(\)\*\+\?\.\\\^\$\|]/g,"\\$&")}
...
$scope.map.setCenter($scope.settings.center);$scope.mapCircle=new google.maps.Circle({map:$scope.map,radius:$scope.settings.radius});$scope.mapCircle.setCenter($scope.settings.center);$scope.mapCircle.setOptions({fillOpacity:CIRCLE_FILL_OPACITY,strokeOpacity:CIRCLE_STROKE_OPACITY,strokeWeight:1,fillColor:"#4169e1",strokeColor:"#0432ff"});$scope.map.fitBounds($scope.mapCircle.getBounds());resetMarkers();_.forEach($scope.filtered_ads,function(ad){if(ad.position){var content="<b>"+ad.title+"</b><br>";var pic_src=_.isArray(ad.pictures)&&ad.pictures.length>0&&_.trim(_.get(ad.pictures[0],"url",""));if(pic_src.length>0){content+="<br><img src=\""+pic_src+"\" style=\"max-width: 150px; max-height: 150px;\"><br>"}
content+="<p>"+ad.short_description+"</p><b>"+ad.formatted_price+"</b>";$scope.markers.push(GoogleMaps.addMarker(_.merge({title:content,action:{label:$translate.instant("View details"),onclick:function(){$scope.showAd(ad)}}},ad.position)))}})}
$scope.showAd=function(ad){$state.go("classifieds-view-item",{value_id:$value_id,item_id:ad.id})};function cleaned_search_options(){var options={query:null,min_price:null,max_price:null,zip:null,bookmarks_only:null,with:{pictures:null,phone:null,address:null},category:null,order:DEFAULT_ORDER};if(!_.isObject($scope.search_options)){$scope.search_options={}}
...
if(!$scope.ad.hide_address){$scope.adblocks.address={label:$scope.ad.title,address:$scope.ad.address,show_address:!0,latitude:$scope.ad.address_lat,longitude:$scope.ad.address_lng,show_geolocation_button:!0}}
if(!$scope.ad.hide_phone){$scope.adblocks.phone={type_id:"phone",content:$scope.ad.phone,icon:[]}}
...
Dialog.alert($translate.instant("Error"),data.message)}).finally(function(){$ionicLoading.hide()})};scope.$on("modal.hidden",function(event,modal){if(modal===$scope.contactModal){$scope.contactModal=!1;try{modal.remove()}catch(e){}}});$ionicModal.fromTemplateUrl('features/classifieds/assets/templates/modals/contact.html',{scope:scope,animation:'slide-in-up'}).then(function(modal){$scope.contactModal=modal;$scope.contactModal.show()})};$scope.$on(AUTH_EVENTS.loginStatusChanged,loadData);$scope.$on("$destroy",$rootScope.$on(Classifieds.EVENTS.ADS_UPDATED,function(){loadData()}))});Features.insertCSS(".classifieds .ng-hide-animate,.classifieds.ng-hide-animate,.classifieds .ng-hide,.classifieds.ng-hide,#classifieds_subheader .ng-hide-animate,#classifieds_subheader.ng-hide-animate,#classifieds_subheader .ng-hide,#classifieds_subheader.ng-hide,#classifieds_footer .ng-hide-animate,#classifieds_footer.ng-hide-animate,#classifieds_footer .ng-hide,#classifieds_footer.ng-hide{-webkit-transition:none!important;transition:none!important;-webkit-animation:none 0s;animation:none 0s}.classifieds .fab{border:none;font-size:1.5em;border-radius:50%;width:50px;height:50px;margin:auto;-webkit-box-shadow:2px 3px 3px 0 rgba(41,41,41,.3);-moz-box-shadow:2px 3px 3px 0 rgba(41,41,41,.3);box-shadow:2px 3px 3px 0 rgba(41,41,41,.3);position:absolute;bottom:20px;right:20px}.classifieds .fab ion-spinner>svg{margin-top:11px}.classifieds #mapContainer,.classifieds #map{position:absolute;left:0;top:0;width:100%;height:100%}.classifieds #map .button{min-height:24px;max-height:24px;padding:5px;line-height:14px;font-size:12px!important}#classifieds_subheader .buttons-right{margin-top:-2px;margin-left:12px;opacity:1;transition:opacity 0.375s,margin-right 0.45s}#classifieds_subheader .buttons-right .icon{margin:-7px}#classifieds_subheader .buttons-right .icon:before{line-height:29px!important;margin-left:-2px}#classifieds_subheader .buttons-right .button-bar .button:first-child{border-radius:4px 0 0 4px}#classifieds_subheader .buttons-right .button-bar .button:last-child{border-radius:0 4px 4px 0}#classifieds_subheader{height:50px;border-top-color:transparent;flex-wrap:wrap;justify-content:flex-start;align-content:flex-start;align-items:flex-start;padding-top:9px;top:44px;transition:height 0.280s ease 0.035s}.platform-ios.platform-cordova:not(.fullscreen) #classifieds_subheader{top:64px}#classifieds_subheader.expanded{top:0;height:100%;padding-top:53px;padding-bottom:39px;transition-delay:0s}.platform-ios.platform-cordova:not(.fullscreen) #classifieds_subheader.expanded{top:0;padding-top:73px;padding-bottom:40px}#classifieds_subheader .item-input-wrapper{width:100%;transition:width 0.3s}#classifieds_subheader.expanded .buttons-right{opacity:0;margin-right:-80px}#classifieds_subheader form{width:100%}#classifieds_subheader .search_row{width:100%;opacity:0;transition:opacity 0.3s ease 0.05s;margin-left:0;margin-top:15px;margin-bottom:0}#classifieds_subheader ion-scroll{height:100%;width:100%}#classifieds_subheader.expanded .search_row{opacity:1}#classifieds_subheader #search_filters{flex-wrap:wrap;display:flex}#classifieds_subheader #search_filters label{font-weight:500}#classifieds_subheader #search_filters .item{width:150px;flex-grow:1}#classifieds_footer .button.small{max-width:32px;padding:1px 6px}#classifieds_footer .button .icon:before,#classifieds_footer .button:before{line-height:31px!important;padding-left:0!important}#classifieds_subheader #search_filters .item input:not([type=\"radio\"]):not([type=\"checkbox\"]),#classifieds_subheader #search_filters .item select{width:100%}#classifieds_subheader #search_filters .full-width{width:100%}#classifieds_subheader #search_filters #withs{display:flex;flex-wrap:wrap}#classifieds_subheader #search_filters #withs label.with{font-weight:400;display:block;padding:10px;line-height:20px;flex-grow:1}.platform-android #classifieds_subheader #search_filters #withs label input,.platform-ios #classifieds_subheader #search_filters #withs label input,.platform-android #classifieds_subheader #bookmarked_ads,.platform-ios #classifieds_subheader #bookmarked_ads{vertical-align:bottom;width:25px;height:25px}#classifieds_subheader #search_filters #withs label input,#classifieds_subheader #bookmarked_ads{margin-bottom:-3px;margin-right:3px}#classifieds_footer{transition:bottom 0.1s}#classifieds_footer.search_expanded{bottom:-45px;transition:bottom 0.1s ease 0.2s}","classifieds")